...
async function recordAIUsage(userId, userType, modelName, functionName, tokenConsumed = 0) {
  try {
    logger.info('[recordAIUsage] 开始记录:', { userId, userType, modelName, functionName, tokenConsumed });
    ...
    logger.info('[recordAIUsage] 查询结果:', existing.length > 0 ? '有记录，更新' : '无记录，插入');
    ...
    if (existing.length > 0) {
      ...
      logger.info('[recordAIUsage] 更新成功, ID:', existing[0].id);
    } else {
      // 插入新记录
      ...
      logger.info('[recordAIUsage] 插入成功, ID:', result.insertId);
    }
  } catch (error) {
    logger.error("[recordAIUsage] 记录AI使用统计失败:", error);
    // 不阻塞主流程
  }
}

// 普通 AI 聊天接口
router.post("/chat", authorize(["0", "1", "2", "3", "4"]), validateAIChat, async (req, res) => {
...
// SSE 流式 AI 聊天接口
router.post("/chat-stream", authorize(["0", "1", "2", "3", "4"]), validateAIChat, async (req, res) => {
  const { prompt, model = "deepseek-chat" } = req.body;
  const userId = req.user?.id;
  const userType = req.user?.role || 'teacher';
  ...
  try {
    ...
    if (userId) {
      ...
      logger.info('[AI统计] 记录完成');
    } else {
      logger.info('[AI统计] userId为空，跳过记录');
    }
...
// 生成会议纪要接口
router.post("/meeting-summary", authorize(["0", "1", "2", "3", "4"]), validateMeetingSummary, async (req, res) => {
...
// 翻译接口
router.post("/translate", authorize(["0", "1", "2", "3", "4"]), validateTranslate, async (req, res) => {
...
翻译：`;
    ...
    logger.error("Error handling translation request:", error);
    return res.status(500).json({
      code: 500,
...
router.post("/editor-completion", authorize(["0", "1", "2", "3", "4"]), validateEditorCompletion, async (req, res) => {
  const { prompt, model = "deepseek-chat" } = req.body;
  const userId = req.user?.id;
  const userType = req.user?.role || 'teacher';
...
6. 内容要专业、准确，适合教育场景`;
    ...
    logger.error("Error handling editor completion request:", error);
    res.write(`event: error\n`);
    res.write(
      `data: ${JSON.stringify({
        code: 500,
...
// AI 智能生成打印材料接口
router.post("/generate-print", authorize(["0", "1", "2", "3", "4"]), validateGeneratePrint, async (req, res) => {
...
// AI 智能生成教案接口（SSE 流式）
router.post("/generate-lesson-plan", authorize(["2", "3", "4"]), validateGenerateLessonPlan, async (req, res) => {
  const { prompt, model = "deepseek-chat" } = req.body;
  const userId = req.user?.id;
  const userType = req.user?.role || 'teacher';
